...
public class HttpUtils {
	...
	public static HttpClient getClient() {
		...
		try {
			HttpConnectionManagerParams mgrParams = client
	...
	public static String doGet(String url, Map<String, String> paramMap) {
		...
		try {
			...
			HttpMethodRetryHandler myRetryHandler = new DefaultHttpMethodRetryHandler(
					3, false);
		...
		try {
			...
			if (params != null && !params.isEmpty()) {
				for (Map.Entry<String, String> entry : params.entrySet()) {
					NameValuePair simcard = new NameValuePair(entry.getKey(),
							entry.getValue());
					list.add(simcard);
				}
				postMethod.setRequestBody((NameValuePair[]) list
						.toArray(new NameValuePair[list.size()]));
			}
			HttpMethodParams hmParams = postMethod.getParams();
			hmParams.setParameter(HttpMethodParams.RETRY_HANDLER,
					myRetryHandler);
